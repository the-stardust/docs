...
func init() {
	...
	fileTypeMap.Store("010000006c000000", "emf") //Real Audio (ram)
...
func GetFileType(fSrc []byte) string {
	...
	if fileType == "" {
		log.Println("未知得文件格式", fileCode[:20])
	}
	return fileType
}
func GetFileTypePrefix(path string) string {
	srcFile, err := os.Open(path)
	if err != nil {
		log.Println(err)
	}
	defer srcFile.Close()
	var headerByte []byte = make([]byte, 20)
	_, err = srcFile.Read(headerByte)
	if err != nil {
		log.Println(err)
	}
