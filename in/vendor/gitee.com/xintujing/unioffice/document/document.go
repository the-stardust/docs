...
import (
	...
	"gitee.com/xintujing/unioffice/common/license"
	"gitee.com/xintujing/unioffice/zippkg"

	"gitee.com/xintujing/unioffice/schema/soo/dml"
	st "gitee.com/xintujing/unioffice/schema/soo/ofc/sharedTypes"
	"gitee.com/xintujing/unioffice/schema/soo/pkg/relationships"
		...
		for i, f := range files {
			...
			if f.Name == target {
				...
				if err != nil {
					...
					//todo 这里没有直接返回错误，而是直接读取无法解析的图片，获得路径直接上传到七牛
					img = common.Image{}
					img.Path = path

					//读取后缀
					imgExtArr := strings.Split(f.Name, ".")
					imgFmt := "png"
					if len(imgExtArr) >= 2 {
						imgFmt = imgExtArr[len(imgExtArr)-1]
					}
					img.Format = imgFmt

					//fmt.Println(path, rel.IdAttr, imgFmt)
				}
