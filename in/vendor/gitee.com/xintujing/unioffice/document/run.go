...
import (
	...
	pic "gitee.com/xintujing/unioffice/schema/soo/dml/picture"
...
//name: zhexiao(肖哲)
//date: 2019-11-05
//从doc xml中读取ruby数据，（主要是拼音数据）
//================================start
func (r Run) Ruby() *CT_Ruby {
	ret := New_CT_Ruby()
	for _, ic := range r.x.EG_RunInnerContent {
		if ic.Ruby == nil {
			continue
		...
		//读取对象数据
		for _, rt := range ic.Ruby.Rt.R.EG_RunInnerContent {
			ret.Rt = append(ret.Rt, rt.T.Content)
		}

		for _, rbs := range ic.Ruby.RubyBase.R.EG_RunInnerContent {
			ret.RubyBase = append(ret.RubyBase, rbs.T.Content)
		}
	}
