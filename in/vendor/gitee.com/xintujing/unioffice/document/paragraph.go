...
func (p Paragraph) Runs() []Run {
	...
	for _, c := range p.x.EG_PContent {
		for _, rc := range c.EG_ContentRunContent {
			...
			//添加
			for _, egr := range rc.EG_RunLevelElts {
				for _, egm := range egr.EG_MathContent {
					buf := bytes.NewBuffer(nil)
					xmle := xml.NewEncoder(buf)
					start := xml.StartElement{}
					var mathStr = ""
					if egm.OMath != nil {
						egm.OMath.MarshalXML(xmle, start)
						// mathStr = `<m:oMathPara xmlns="http://schemas.openxmlformats.org/officeDocument/2006/math" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math" xmlns:s="http://schemas.openxmlformats.org/officeDocument/2006/sharedTypes" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:xml="http://www.w3.org/XML/1998/namespace">` + buf.String() + "</m:oMath>" + "</m:oMathPara>"

						mathStr = buf.String() + "</m:oMath>"
					}
					if egm.OMathPara != nil {
						egm.OMathPara.MarshalXML(xmle, start)
						mathStr = buf.String() + "</m:oMathPara>"
					}
					r := wml.NewCT_R()
					run := Run{p.d, r}
					run.AddText(mathStr)
